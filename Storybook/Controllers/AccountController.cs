...
namespace Storybook.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                var user = await AccountManager.FindAsync(model.UserName, model.Password);
                if (user != null)
                {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new User { UserName = model.UserName, Name = model.Name ?? model.UserName };
                IdentityResult result = await AccountManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
        ...
        private async Task SignInAsync(User user, bool isPersistent)
        {
            ...
            AuthenticationManager.SignIn(new AuthenticationProperties { IsPersistent = isPersistent }, await AccountManager.GenerateUserIdentityAsync(user));
        }
